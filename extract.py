...
class ARTAvatarInferEngine:
    ...
    def rendering(self, audio, pred_motions, shape_id="mesh", shape_code=None, save_name='ARTAvatar.mp4'):
        ...
        if shape_id == "mesh":
            ...
            verts = self.ARTalk.basic_vae.get_flame_verts(self.flame_model, shape_code, pred_motions, with_global=True) # with global은 base transformation을 적용할지 말지를 의미한다. True면 한다는 것임.
            #TODO save verts or save mesh
            """
            >>> verts.shape (already transform with base head transformation)
                torch.Size([340, 5023, 3])
            """
            # import code; code.interact(local=locals())

            np_verts = verts.detach().cpu().numpy()
            np.save(f"./{self.npy_dir}/{save_name}.npy", np_verts)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--style_id", '-s', default='natural_3', type=str)
